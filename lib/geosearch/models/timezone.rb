  ...
  class Timezone < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `GeoSearch::Timezone` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `GeoSearch::Timezone`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'dst_offset')
        self.dst_offset = attributes[:'dst_offset']
      end

      if attributes.key?(:'raw_offset')
        self.raw_offset = attributes[:'raw_offset']
      end
    end
    ...
    def hash
      [id, country_code, timezone_id, gmt_offset, dst_offset, raw_offset].hash
    end
